...
function RegisterAccount (prop) {
  ...
  useEffect(() => {
    if (session) {
      const res = async () => await fetch('/api/profile', { method: 'POST', body: JSON.stringify({ id: session?.user?._id }) })
      res().then((res) => {
        res.json().then(async (data) => {
          const update = await fetch('/api/connect/conn_messenger', {
            method: 'POST',
            ...
            body: JSON.stringify({
              ...
              webhookPath: `${server}/messenger/${data.profile?.company}/webhook`
            })
          })
  ...
  const submitchange = async (e) => {
    try {
      const verify = await fetch(`https://graph.facebook.com/me?access_token=${getValues('token_auth')}`)
            ...
            res.json().then(async (data) => {
              const update = await fetch('/api/connect/conn_messenger', {
                ...
                body: JSON.stringify({
                  ...
                  idPage: getValues('id_number'),
                  tokenAuthorization: getValues('token_auth'),
                  tokenWebhook: getValues('token_web'),
                  webhookPath: `${server}/messenger/${data.profile?.company}/webhook`
                })
              })

              if (update?.ok) {
                const res = async () => await fetch('/api/edit/company', { method: 'POST', body: JSON.stringify({ ...data.profile, id: data.profile._id, messenger: true }) })
                res().then((res) => {
    ...
    <div className='p-4 row align-self-center justify-content-center bg-secondary-subtle rounded w-75 h-75'>
        ...
        {prop.status
          ? generate
            ...
            <div className='row'>
                <label className='label col-12 p-0'>Id de Página</label>
